Title: SPIFFE | SPIRE Concepts

URL Source: https://spiffe.io/docs/latest/spire-about/spire-concepts/
...
SPIRE is a production-ready implementation of the [SPIFFE APIs](https://spiffe.io/docs/latest/spiffe/overview/) that performs node and workload attestation in order to securely issue SVIDs to workloads, and verify the SVIDs of other workloads, based on a predefined set of conditions.
...
![Image 1: Server and Agent](https://spiffe.io/img/server_and_agent.png)

 Server and Agent 

## All about the Server[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#all-about-the-server)

A SPIRE Server is responsible for managing and issuing all identities in its configured SPIFFE trust domain. It stores [registration entries](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-registration) (which specify the selectors that determine the conditions under which a particular SPIFFE ID should be issued) and signing keys, uses [node attestation](https://spiffe.io/docs/latest/spire-about/spire-concepts/#node-attestation) to authenticate agents’ identities automatically, and creates SVIDs for workloads when requested by an authenticated agent.

![Image 2: The SPIRE Server](https://spiffe.io/img/server.png)
...
**Node attestor plugins** which, together with agent node attestors, verify the identity of the node the agent is running on. See the section [Node Attestation](https://spiffe.io/docs/latest/spire-about/spire-concepts/#node-attestation) for more information.

**Datastore plugins**, which the server uses to store, query, and update various pieces of information, such as [registration entries](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-registration), which nodes have attested, what the selectors for those nodes are. There is one built-in datastore plugin which can use a MySQL, SQLite 3, or PostgresSQL database to store the necessary data. By default it uses SQLite 3.
...
You customize the server’s behavior by configuring plugins and various other configuration variables. See the [SPIRE Server Configuration Reference](https://spiffe.io/docs/latest/deploying/spire_server/) for details.

## All About the Agent[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#all-about-the-agent)
...
![Image 3: The SPIRE Agent](https://spiffe.io/img/agent.png)

 The SPIRE Agent 

The agent’s main components include:

*   Node attestor plugins which, together with server node attestors, verify the identity of the node the agent is running on. See the section [Node Attestation](https://spiffe.io/docs/latest/spire-about/spire-concepts/#node-attestation) for more information.

*   Workload attestor plugins – which verify the identity of the workload process on the node, by querying information about the process from the node operating system and comparing it against the information you gave the server when you [registered the workload’s properties](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-registration) using selectors. See the section [Workload Attestation](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-attestation) for more information.

*   Key manager plugins, which the agent uses to generate and use private keys for X.509-SVIDs issued to workloads.

You customize the agent’s behavior by configuring plugins and other configuration variables. See the [SPIRE Agent Configuration Reference](https://spiffe.io/docs/latest/deploying/spire_agent/) for details.

## Custom Server and Agent Plugins[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#custom-server-and-agent-plugins)

You can create custom server and agent plugins for particular platforms and architectures for which SPIRE doesn’t include plugins. For example, you could create server and agent node attestors for an architecture other than those summarized under [Node Attestation](https://spiffe.io/docs/latest/spire-about/spire-concepts/#node-attestation). Or you could create a custom keymanager plugin to handle private keys in a way SPIRE doesn’t currently support. Because SPIRE loads custom plugins at runtime, you do not need recompile SPIRE in order to enable them.
...
7.   The agent presents this proof of identity to the server over a TLS connection authenticated via the bootstrap bundle the agent is configured with. This bootstrap bundle is a default configuration, and should be replaced with customer-supplied credentials in production. 
...
14.   The server then fetches all [authorized registration entries](https://spiffe.io/docs/latest/spire-about/spire-concepts/#authorized-registration-entries) from its data store and sends them to the agent.
...
## Workload Registration[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-registration)
...
Workload registration is covered in detail in the [SPIRE Documentation](https://spiffe.io/docs/latest/spire/using/registering/)
...
### Node Attestation[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#node-attestation)
...
![Image 4: Node Attestation](https://spiffe.io/img/node_attestation.png)

 Node Attestation 

#### Summary of Steps: Node Attestation[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#summary-of-steps-node-attestation)
...
#### Node Attestors[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#node-attestors)
...
**using an existing X.509 certificate** – For information on configuring node attestors, see the [SPIRE Server Configuration Reference](https://spiffe.io/docs/latest/deploying/spire_server/) and [SPIRE Agent Configuration Reference](https://spiffe.io/docs/latest/deploying/spire_agent/).
...
These properties are then compared against the information provided to the server when you [registered the workload’s properties using selectors](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-registration).
...
![Image 5: Workload Attestation](https://spiffe.io/img/workload_attestation.png)

 Workload Attestation 

#### Summary of Steps: Workload Attestation[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#summary-of-steps-workload-attestation)
...
#### Workload Attestors[](https://spiffe.io/docs/latest/spire-about/spire-concepts/#workload-attestors)

SPIRE includes workload attestor plugins for Unix, Kubernetes, and Docker. For details on specific attestors supported by default, see the section [Attestation](https://spiffe.io/docs/latest/spire-about/spire-concepts/#attestation).

![Image 6](https://static.scarf.sh/a.png?x-pxid=ac2d227e-ec4b-4022-a4ae-01f4bdd38126)
